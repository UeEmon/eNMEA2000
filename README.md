...
本体Webの公開ポートはホスト側18081番／コンテナ内80番です。80番はコンテナのネットワーク名前空間で非rootユーザーが待ち受けられるよう設定しています。
...
画面右上の「保存データ削除」では、現在の件数を確認し「全件削除」と入力した後、DB内の受信履歴とファイル解析ジョブ情報を全件削除します。削除中は受信・新規アップロードを止め、解析中のジョブがある場合は削除できません。削除後も受信は停止したままです。アップロードした元ファイルはディスクに残り、DBのテーブル構造と認証設定も保持します。履歴削除は復元できないため、必要なら先に出力またはバックアップを行ってください。API `DELETE /api/data` はログインに加え、`{"confirm":"全件削除","expected_total":現在の件数}` の指定が必要です。
